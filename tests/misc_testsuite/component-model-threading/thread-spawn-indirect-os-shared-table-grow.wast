...
;;
;; The parent grows the shared start table, writes a parent VMFuncRef into the
;; newly allocated slot, and then spawns that new index. The unsafe OS-thread
;; path must accept growable imported start tables and remap the selected
;; parent VMFuncRef to the sibling Store's counterpart before calling it.

(component
  (core module $libc
    (memory (export "mem") 1 1 shared)
    (table (export "__indirect_function_table") shared 2 3
  ...
  (core module $m
    ...
    (import "libc" "__indirect_function_table"
      (table $indirect-function-table shared 2 3 (ref null (shared func))))
    ...
    (func (export "run") (type $run-ty)
      ...
      (if (i32.ne
          (table.grow $indirect-function-table
        ...
        (then
          (return (i32.const 90))))
      (if (i32.ne
          (table.size $indirect-function-table)
          (i32.const 3))
        (then
          (return (i32.const 91))))
      (drop (call $thread-spawn-indirect (i32.const 2) (i32.const 0)))
      (loop $again
