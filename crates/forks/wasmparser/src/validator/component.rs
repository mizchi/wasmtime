...
impl ComponentState {
    ...
    pub fn canonical_function(
        ...
        match func {
            ...
            CanonicalFunction::ThreadSpawnRef { func_ty_index } => {
                self.thread_spawn_ref(true, func_ty_index, types, offset)
            }
            CanonicalFunction::ThreadSpawnIndirect {
                func_ty_index,
                table_index,
            } => self.thread_spawn_indirect(true, func_ty_index, table_index, types, offset),
            CanonicalFunction::ThreadAvailableParallelism => {
                self.thread_available_parallelism(true, types, offset)
            }
    ...
    fn thread_index(&mut self, types: &mut TypeAlloc, offset: usize) -> Result<()> {
        ...
        let core_ty = SubType::func(FuncType::new([], [ValType::I32]), true);
    ...
    fn thread_spawn_ref(
        ...
        let core_type_id = self.validate_spawn_type(shared, func_ty_index, types, offset)?;
        ...
        let core_ty = SubType::func(func_ty, shared);
        let id = types.intern_sub_type(core_ty, offset);
        self.core_funcs.push(id);
    ...
    fn thread_spawn_indirect(
        ...
        let _ = self.validate_spawn_type(shared, func_ty_index, types, offset)?;
        ...
        let expected_element_type = if shared {
            RefType::FUNCREF
                .shared()
        ...
        .map_err(|mut e| {
            ...
            let func = if shared.is_empty() {
                "func"
            } else {
                "shared func"
            };
            e.add_context(format!(
                "table is not a 32-bit {shared}table of (ref null ({func}))"
            ));
            e
        })?;
    ...
    fn validate_spawn_type(
        ...
        let sub_ty = &types[core_type_id];
        if sub_ty.composite_type.shared != shared {
            bail!(
                offset,
                "spawn type sharedness must match canonical thread spawn sharedness"
            );
        }
        match &sub_ty.composite_type.inner {
            CompositeInnerType::Func(func_ty) => {
    ...
    fn instantiate_core_module(
        ...
        for ((module, name), expected) in module_type.imports.iter() {
            ...
            if Self::fork_local_thread_index_shared_match(name.as_str(), arg, expected, types) {
                continue;
            }
    ...
    fn fork_local_thread_index_shared_match(
        ...
        let (EntityType::Func(actual), EntityType::Func(expected)) = (actual, expected) else {
            return false;
        };
        let actual = &types[*actual];
        let expected = &types[*expected];
        if actual.composite_type.shared == expected.composite_type.shared {
            return false;
        }

        let (CompositeInnerType::Func(actual), CompositeInnerType::Func(expected)) =
            (&actual.composite_type.inner, &expected.composite_type.inner)
        else {
            return false;
        };

        actual.params().is_empty()
            && expected.params().is_empty()
            && actual.results() == [ValType::I32]
            && expected.results() == [ValType::I32]
    }
